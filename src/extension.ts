...
async function openApiKeySettings(context: vscode.ExtensionContext): Promise<void> {
    const panel = vscode.window.createWebviewPanel(
        'tridentApiKeySettings',
        'Add AI Insights',
        vscode.ViewColumn.One,
        { enableScripts: true }
    );
    ...
    const hasConsent = context.globalState.get<string>('gooseConsent') === 'enabled';
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.command === 'saveApiKey' && typeof msg.apiKey === 'string') {
            ...
            const consent = context.globalState.get<string>('gooseConsent') === 'enabled';
            ...
            panel.webview.html = getApiKeySettingsWebviewContent(false, context.globalState.get<string>('gooseConsent') === 'enabled');
...
async function runGooseCliInstall(): Promise<void> {
...
function getApiKeySettingsWebviewContent(hasKey: boolean, hasConsent: boolean): string {
...
async function runNpmAudit(panel: vscode.WebviewPanel, projectRoot: string): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
        ...
        const disposable = panel.webview.onDidReceiveMessage((msg: { command?: string }) => {
...
function getGooseConfig() {
    ...
    return {
        ...
        timeoutMs: cfg.get<number>('timeoutMs', 120000),
...
function resolveRecipePathForExtension(recipePath: string, projectRoot?: string): string {
    if (path.isAbsolute(recipePath)) return recipePath;
...
function findLastJsonObjectSpan(input: string): { start: number; end: number } | null {
    let inString = false;
    let escape = false;
    let depth = 0;
    let start = -1;
    let lastSpan: { start: number; end: number } | null = null;
...
function parseGooseOutput(raw: string): unknown {
...
function logGooseParseFailure(raw: string, err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
...
async function runSecureGooseWithRetry(
    ...
    const provider = gooseConfig.get<string>('provider', 'openrouter');
    const model = gooseConfig.get<string>('model', 'openai/gpt-5.1');
...
export async function onVulnSelected(vuln: VulnerabilitySelection) {
    ...
    const hasApiKey = extensionContext?.globalState.get<boolean>('trident.hasApiKey') === true;
    if (!hasApiKey) {
        let vulnIdForMsg = 'unknown';
        try { vulnIdForMsg = sanitizeId(String(vuln.id || `${vuln.packageName || 'pkg'}:${vuln.version || 'unknown'}:${vuln.title || 'vuln'}`)); } catch { /* use unknown */ }
    ...
    try { vulnIdForStatus = sanitizeId(String(vuln.id || `${vuln.packageName || 'pkg'}:${vuln.version || 'unknown'}:${vuln.title || 'vuln'}`)); } catch { /* use unknown */ }
    const sendStatus = (status: string) => sendToWebview({ type: 'gooseInsight', vulnId: vulnIdForStatus, data: { pending: true, status } });
