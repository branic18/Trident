import { GooseVulnInsight, CodeFix } from './types';

/**
 * Validates and sanitizes Goose AI output to prevent malicious content
 */

export interface GooseVulnInsightValidator {
...
export class JsonSchemaValidator implements GooseVulnInsightValidator {
  ...
  private validateImpact(impact: unknown): string {
  ...
  private validatePriorityScore(score: unknown): number {
  ...
  private validatePriorityReason(reason: unknown): string {
  ...
  private validateRecommendedActions(actions: unknown): string[] {
  ...
  private validateFixStyle(fixStyle: unknown): string {
  ...
  private validateDevSummary(summary: unknown): string {
  ...
  private validateCodeContent(content: unknown, field: string): string {
  ...
  private validateCodeDescription(description: unknown): string {
  ...
  private validateWarnings(warnings: unknown): string[] {
  ...
  filterSuspiciousContent(insight: GooseVulnInsight): GooseVulnInsight {
    ...
    filtered.recommendedActions = filtered.recommendedActions.map(action => 
 ...
 */
export function createGooseValidator(): GooseVulnInsightValidator {
